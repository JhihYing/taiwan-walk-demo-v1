...
function getTourPage() {
  axios
    .get(
      `https://tdx.transportdata.tw/api/basic/v2/Tourism/ScenicSpot?%24filter=ScenicSpotID%20eq%20%27${id}%27&%24format=JSON`,
      {
    ...
    .then(function (response) {
      ...
      iframe.setAttribute(
        "src",
        `https://www.google.com/maps/embed/v1/place?key=${config.apiKey}&q=${thisData.ScenicSpotName}&zoom=17&center=${thisData.Position.PositionLat},${thisData.Position.PositionLon}`
      );
      ...
      nearbyTourism.setAttribute(
        "href",
        `./nearby-tourism.html?theme=探索景點=${thisData.ScenicSpotID}`
      );
      nearbyFestival.setAttribute(
        "href",
        `./nearby-festival.html?theme=探索景點=${thisData.ScenicSpotID}`
      );
      nearbyFood.setAttribute(
        "href",
        `./nearby-food.html?theme=探索景點=${thisData.ScenicSpotID}`
      );
...
function getTourPageMore() {
  axios
    .get(
      `https://tdx.transportdata.tw/api/basic/v2/Tourism/ScenicSpot/${city}?%24format=JSON`,
      {
        headers: getAuthorizationHeader(),
      }
    )
